...
const AppContent: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getPageTitle = () => {
    const routeTitles: { [key: string]: string } = {
        ...
        '/dashboard/external-link': 'External Link',
    };
  ...
  return (
      ...
      <div className={`flex h-screen bg-secondary ${isMobileMenuOpen ? 'mobile-menu-open' : ''}`}>
        <Sidebar
          isCollapsed={isMobileMenuOpen ? false : isSidebarCollapsed}
          isMobileOpen={isMobileMenuOpen}
          onMobileClose={closeMobileMenu}
        />
        <div className={`flex-1 flex flex-col overflow-hidden ${isMobileMenuOpen ? 'main-content-mobile' : ''}`}>
          <header className={`main-header flex items-center justify-between h-16 px-4 ${isMobileMenuOpen ? 'mobile-blur' : ''}`}>
            <div className="flex items-center">
              ...
              >
                <div className="flex flex-col items-center justify-center space-y-1">
                  <div className={`w-5 h-0.5 bg-gray-600 transition-all duration-200 ${isSidebarCollapsed ? 'rotate-45 translate-y-1.5' : ''}`}></div>
                  <div className={`w-5 h-0.5 bg-gray-600 transition-all duration-200 ${isSidebarCollapsed ? 'opacity-0 scale-0' : ''}`}></div>
                  <div className={`w-5 h-0.5 bg-gray-600 transition-all duration-200 ${isSidebarCollapsed ? '-rotate-45 -translate-y-1.5' : ''}`}></div>
                </div>
                <div
                  className={`absolute -top-1 -right-1 w-3 h-3 rounded-full transition-all duration-200 border border-white shadow-sm ${
                    isSidebarCollapsed ? 'bg-red-500 scale-110' : 'bg-green-500 scale-100'
                  }`}
                ></div>
              </button>
              <div className="mobile-menu-button md:hidden">
                <Hamburger isActive={isMobileMenuOpen} onClick={toggleMobileMenu} />
              </div>
